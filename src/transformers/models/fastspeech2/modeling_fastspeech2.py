...
class FastSpeech2Embeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = FastSpeech2PositionalEmbedding(config.max_position_embeddings, config.hidden_size, padding_idx=config.pad_token_id)
...
class FastSpeech2Layer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, _, = self.attention(self.LayerNorm1(hidden_states), attention_mask=attention_mask)
        hidden_states = residual + hidden_states
        hidden_states = hidden_states * (1 - attention_mask.float()).transpose(0, 1)[..., None]

        residual = hidden_states
        hidden_states = self.output(self.intermediate(self.LayerNorm2(hidden_states)), residual)
        hidden_states = hidden_states * (1 - attention_mask.float()).transpose(0, 1)[..., None]
...
class FastSpeech2ForMultipleChoice(FastSpeech2PreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        FASTSPEECH2_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length")
    )
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
